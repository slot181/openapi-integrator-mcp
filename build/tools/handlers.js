...
import { ensureDirectoryExists, isValidHttpUrl, downloadFile, uploadToCfImgbed, handleSiliconFlowVideoGeneration, sendImageUploadNotification,
            ...
            case 'generate_image': {
                ...
                for (const item of responseData) {
                    ...
                    if (Date.now() - startTime > config.imageProcessingTimeout) {
                        ...
                        throw new McpError(ErrorCode.InternalError, // Or a custom code? -32001?
                        `Image processing timed out after ${config.imageProcessingTimeout / 1000} seconds. Task continues in background via notifications (if configured).`, { partialResults: results } // Optionally include partial results if needed
                        );
                    }
